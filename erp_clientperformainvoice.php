<?php
  session_start();
  $pagetitle = "業務部 &raquo; Performa Invoice";
  include("_data.php");
  include("_erp.php");
  ...
  if (($_GET["submit"]=="Excel") or  ($_GET["submit"]=="PDF")) {  
      ...
      //$type=$row2['OCCUD11'];         
      $filename="templates/performainvoice.xls";   
      ...
      $objPHPExcel ->getProperties()->setCreator( 'Performa Invoice')
                   ->setLastModifiedBy('Performa Invoice')
                   ->setTitle('Performa Invoice')
                   ->setSubject('Performa Invoice')
                   ->setDescription('Performa Invoice')
                   ->setKeywords('Performa Invoice')
                   ->setCategory('Performa Invoice'); 
      ...
      $objPHPExcel->setActiveSheetIndex(0);
        ...
        header('Content-Disposition: attachment;filename="performa_invoice_'. $bocc01 . '_'.$bdate.'.xls"');    
