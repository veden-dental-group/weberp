<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內每日到貨統計V4";
  include("_data.php");
  include("_erp.php");
  //auth("erp_alldaterangecasereportv4.php");  
  ...
  if ($_GET["submit"]=="匯出") {
     ...
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      $s2="select oea02, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select to_char(oea02,'yyyy-mm-dd') oea02, ta_oea006, ta_oea011, ta_oea004 from oea_file where oea02 between to_date('$bdate','yy/mm/dd') 
             and oea04='$occ01' and to_date('$edate','yy/mm/dd')  )a ) group by oea02 order by oea02";
      $erp_sql2 = oci_parse($erp_conn1,$s2 );
      oci_execute($erp_sql2);         
      $y=3;   
      $t=array('    ','A1','A2');
      ...
      header('Content-Disposition: attachment;filename="Dailysum_byCustomer_in.xls"');
