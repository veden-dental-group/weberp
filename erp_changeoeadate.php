<?
  ...
  if (is_null($_GET['neworderdate'])) {
    $neworderdate =  date('Y-m-d');
  } else {
    $neworderdate=$_GET['neworderdate'];
  }       
  
  if (is_null($_GET['newduedate'])) {
    $newduedate =  date('Y-m-d');
  } else {
    $newduedate=$_GET['newduedate'];
  }                                         
  ...
  if ($_POST["action"] == "save") {
      ...
      foreach ($_POST["oeaarray"] as $oea01){
          if ($_POST["risok" . $oea01] == "Y") {
              ...
              //vd210  訂單
              $soea= "update oea_file set oea02=to_date('$neworderdate1','yy/mm/dd'), ta_oea005=to_date('$newduedate1','yy/mm/dd') where oea01='$oea01'";
              $erp_sqloea=oci_parse($erp_conn2,$soea);
              ...
              //vd210  採購單
              $soea= "update pmm_file set pmm04=to_date('$neworderdate1','yy/mm/dd') where pmm99='$oea99'";
              $erp_sqloea=oci_parse($erp_conn2,$soea);
              $rs=oci_execute($erp_sqloea);
              if ($rs) {
                  $msg.="更新成功!!";  
              ...
              //vd110 工單
              $soea= "update sfb_file set sfb13=to_date('$neworderdate1','yy/mm/dd'), sfb15=to_date('$newduedate1','yy/mm/dd') where sfb22='$oea01'";   
              $erp_sqloea=oci_parse($erp_conn1,$soea);
              $rs=oci_execute($erp_sqloea);
              if ($rs) {
                  $msg.="更新成功!!";  
    ...
    <table width="100%"  border="0" cellpadding="4" cellspacing="1" style="border: 1px solid #cccccc">
      <tr>
        <td bgcolor="#eeeeee"><div align="left">
            ...
            New Order Date:
            <input name="neworderdate" type="text" id="neworderdate" size="12" maxlength="12" value=<?=$neworderdate;?> onfocus="new WdatePicker()">&nbsp; &nbsp;            
            New Due Date:
            <input name="newduedate" type="text" id="newduedate" size="12" maxlength="12" value=<?=$newduedate;?> onfocus="new WdatePicker()">&nbsp; &nbsp;
