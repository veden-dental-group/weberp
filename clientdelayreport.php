  ...
  if (is_null($_GET['occ01'])) {
    ...
    if ($_GET["submit"]=="匯出") {   
        ...
        while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {   
              ...
              $msg2='';  
                if ($_GET['checkmrp']=='Y'){
                     $qmrp="SELECT k_slipno FROM sk01 WHERE k_engcode = '$rxno' order by k_slipno desc";
                     $idmrp=odbc_exec($mrp_conn,$qmrp);
                     $resultmrp=odbc_fetch_row($idmrp);
                     $slipno=odbc_result($idmrp,1);
                     $qmrp="SELECT p9_date FROM ps09a WHERE p9_skno = '$slipno'";
                     $idmrp=odbc_exec($mrp_conn,$qmrp);
                     $resultmrp=odbc_fetch_row($idmrp);
                     $outdate=odbc_result($idmrp,1);
                     if (!$outdate) {                   //查不到資料會傳回false
                          $msg2="";
                     } else {
                          $msg2="MRP已於" . $outdate . "出貨完畢, 請刪除VD110的派工單及訂單資料";
                     }
                }  
  ...
  <div align="left">
    <table width="100%"  border="0" cellpadding="4" cellspacing="1" style="border: 1px solid #cccccc">
      <tr><td bgcolor="#eeeeee">
        ...
        <input type="checkbox" name="checkmrp" value='Y' <? if ($_GET['checkmrp']=='Y') echo " checked";?> >檢查MRP </input>  &nbsp;&nbsp;   &nbsp;&nbsp;  
