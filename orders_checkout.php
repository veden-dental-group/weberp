<?php
  session_start();
  $pagetitle = "車間作業 &raquo; 出站報工";
  include("_data.php");
  auth("orders_checkout.php");;
  ...
  Function checkinOrders($v){      
      ...
      $erp_sql = oci_parse($erp_conn, "select sfb01 from sfb_file where sfb01='$sfb01'");
      oci_execute($erp_sql);  
      ...
      if ($errmsg!=''){ //有錯誤訊息
          ...
          $s="select tc_srg004 from tc_srg_file, ecb_file where tc_srg001='$sfb01' and tc_srg005 is not null and tc_srg003=ecb01 and tc_srg004=ecb03 and ecb06='$ecb06' " .
             "and ( (tc_srg007 is not null and tc_srg018='N') or (tc_srg019 is not null and tc_srg018='Y') )";
          ...
          if (is_null($row4['TC_SRG004'])){
            ...
            $o->assign('orderstatus','innerHTML',$errmsg);    
             ...
             //本程式不能做最後一站報工
             $erp_sql = oci_parse($erp_conn, "select count(*) l_n from tc_srg_file where tc_srg001='$sfb01' and tc_srg004>'". $row4['TC_SRG004'] ."' ");
             oci_execute($erp_sql);  
             $row7 = oci_fetch_array($erp_sql, OCI_ASSOC);  
             if ($row7['L_N'] ==0 ){
                 $o->alert('本程式不能做最後一站報工!!');
             } else {
                 ...
                 if ($row5['TC_SRG018']=='N') {  //非返工
                    if (is_null($row5['TC_SRG010'])){   //沒出站過   
                        if ($row5['TC_SRG006']=='N'){   // 不用PQC
                            //將本工序在製量清為NULL                   
                            $today=date('Y-m-d');
                            $time=date('h:i:s');
                            $s="update tc_srg_file set tc_srg005=NULL, tc_srg010=to_date('$today','YYYY-MM-DD'), tc_srg011='$time', tc_srg012='$gen01' where tc_srg001='$sfb01' and tc_srg004='". $row4['TC_SRG004'] . "' ";
                            $erp_sql = oci_parse($erp_conn, $s );
                            oci_execute($erp_sql);  
                            $new='Y';                                 
                            
                            //將在製量移到下一個工序     
                            $s="select min(tc_srg004) tc_srg004 from tc_srg_file where tc_srg001='$sfb01' and tc_srg004>'". $row4['TC_SRG004'] . "' ";
                            $erp_sql = oci_parse($erp_conn, $s );
                            oci_execute($erp_sql);  
                            $row6 = oci_fetch_array($erp_sql, OCI_ASSOC);  
                                                                            
                            $s="update tc_srg_file set tc_srg005=". $row5['TC_SRG005'] ." where tc_srg001='$sfb01' and tc_srg004='". $row6['TC_SRG004']."'";
                            $erp_sql = oci_parse($erp_conn, $s );
                            oci_execute($erp_sql);  
                            $new='Y';   
                        } else {
                            $today=date('Y-m-d');
                            $time=date('h:i:s');
                            $s="update tc_srg_file set tc_srg010=to_date('$today','YYYY-MM-DD'), tc_srg011='$time', tc_srg012='$gen01' where tc_srg001='$sfb01' and tc_srg004='". $row4['TC_SRG004'] . "' ";
                            $erp_sql = oci_parse($erp_conn, $s );
                            oci_execute($erp_sql);  
                            $new='Y';            
                        }    
                    ...
                    if (is_null($row5['TC_SRG022'])){       //未報過工
                        if ($row5['TC_SRG006']=='N'){   // 不用PQC
                            //將本工序在製量清為NULL                   
                            $today=date('Y-m-d');
                            $time=date('h:i:s');
                            $s="update tc_srg_file set tc_srg005=NULL, tc_srg022=to_date('$today','YYYY-MM-DD'), tc_srg023='$time', tc_srg024='$gen01' where tc_srg001='$sfb01' and tc_srg004='". $row4['TC_SRG004'] . "' ";
                            $erp_sql = oci_parse($erp_conn, $s );
                            oci_execute($erp_sql);  
                            $new='Y';                                 
                            ...
                            $today=date('Y-m-d');
                            $time=date('h:i:s');
                            $s="update tc_srg_file set tc_srg022=to_date('$today','YYYY-MM-DD'), tc_srg023='$time', tc_srg024='$gen01' where tc_srg001='$sfb01' and tc_srg004='". $row4['TC_SRG004'] . "' ";
                            $erp_sql = oci_parse($erp_conn, $s );
                            oci_execute($erp_sql);  
                            $new='Y';            
                        }    
      ...
      if ($new=='Y'){   //有報過工
        ...
        $ordersvalue=$ordersvalue . '<td width=10%>' . $gen01               . '</td>' ; 
