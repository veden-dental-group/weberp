<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內每日到貨統計";
  include("_data.php");
  include("_erp.php");
  auth("erp_alldaterangecasereport.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
      ...
      $objPHPExcel = $objReader->load("templates/erp_daterangecases.xls");    
          ...
          (select a.oea02, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select to_char(oea02, 'yyyy-mm-dd') oea02, ima01, ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, ima_file
     ...
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      ...
      header('Content-Disposition: attachment;filename="DateRangeCases.xls"');    
