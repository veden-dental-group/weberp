...
$edate = date("Y-m-d", strtotime($tdate) - (86400 * 3));
...
if ($weekday==0) exit;   //星期日不計算delay

if ($weekday==5 or $weekday==6) {  //PTL, LP, F-1 星期五六 不算delay
    $s2= "select oea01, to_char(oea02,'yyyy-mm-dd') oea02, oea04, occ02, ta_oea006, sfb01,sfb22, sfb221, sfb82, gem02, sfb05, ima02, sfb08, imaud07, substr(imaud02,2,1) mday, substr(imaud02,3,1) qday, substr(imaud02,4,3) mplus, substr(imaud02,7,3) rplus, to_char(oeaud13,'yyyy-mm-dd') oeaud13  " .
...
while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
    ...
    if ($ooea01!=$row2['OEA01']) { //新的工單號 在單頭加一筆
        $queryd= "insert into delay ( tdate, orderdate, orderno, rx, clientid, clientname, duedate, oeaud13, stage, makeday, recday, faxday, status ) values (
                 '" . $tdate                        . "',
                 '" . $row2['OEA02']                . "',
                 '" . $row2['OEA01']                . "',
                 '" . $row2['TA_OEA006']            . "',
...
$queryd="select orderno, orderdate, oeaud13 from delay where tdate='$tdate' ";
$resultd=mysql_query($queryd);
while ($rowd=mysql_fetch_array($resultd)){
    $orderno=$rowd['orderno'];

    //取本工單各品代的合計天數, 要判斷顆數是否>加一天的顆數
    //  oracle可用 sign()函?根据某?值是0、正??是??，分?返回0、1、-1 , 因此判斷 qty-qtyday 是否是正數, 如果是, 則加一天, 否則不用加天數
    ...
    if (is_null($rowd['oeaud13']) || $rowd['oeaud13']=='' ) {   //如果業務有另行錄入 oeaud13 則以此日期為 delay 日期
        $duedate=date('Y-m-d',strtotime($orderdate)+$totalseconds);
    } else {
        $duedate=$rowd['oeaud13'];
    }
...
$filename='report/' . $tdate . '_DelayCasesReportv3.xls';
$objWriter->save($filename);


  $report_name = $edate . ' 應出貨但未出貨的RX#';
