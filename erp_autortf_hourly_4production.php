...
//今天寄貨, 累計寄貨, 累計重修, 累計內返
$query1= " (select s13 , sum(s25*s26) sent, 0 tsent, 0 reject, 0 redo, 0 delay  from casesent where s01='$tdate' group by s13) ";  
...
$query = "select s13 , sum(sent) sent, sum(tsent) tsent, sum(reject) reject, sum(redo) redo, sum(delay) delay from " . 
         "(select s13 , sent, tsent, reject, redo, delay from " .
         "( $query1 union all $query2 union all $query3 union all $query4 ) a) b, maker where s13=mid group by mname order by mname  ";
$result= mysql_query($query);
while ($row = mysql_fetch_array($result)){    
    $key=$row['s13'].$guid;
    $todaysent=$row['sent'];
    ...
    $totalredo=$row['redo'];  
    $totaldelay=$row['delay'];  
    $q1 = "update  erp_led set todaysentcase=$todaysent, totalsentcase=$totalsent, totalrejectcase=$totalreject, totalredocase=$totalredo, totaldelaycase=$totaldelay where guid='$key'";        
    $r1 = mysql_query($q1) ;    
} 
...
while ($row=mysql_fetch_array($result)) {
    ...
    $query1="select todayincase, todaysentcase, totalsentcase, totalrejectcase, totalredocase, totaldelaycase, (totalsentcase-totalrejectcase-totalredocase-totaldelaycase) moneycase from erp_led where maker='$maker' order by created desc limit 1"; //取出最新的一筆
