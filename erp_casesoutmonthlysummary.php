<?php
  session_start();
  $pagetitle = "財務部 &raquo; 每月各種產品出貨統計表";
  include("_data.php");
  ...
  if ($_GET["submit"]=="匯出") {   
    ...
    $filename='templates/casesoutmonthlysummary.xls';
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A1', $thisyear.' 年各种产品出货统计');       
    ...
    $s2="select oea02, sum(a1) a1, sum(a2) a2, sum(a3) a3,  sum(a4) a4, sum(b1) b1, sum(b2) b2, sum(b3) b3, sum(b4) b4, sum(b5) b5 from " .
          ...
          "    (select to_char(oga02, 'yyyy-mm') oea02, ima06, (ogb12 * imaud07) total from ogb_file, oga_file, ima_file where ogb01=oga01 and to_char(oga02,'yyyy')='$thisyear' and ogb04=ima01)) " .
    ...
    $objPHPExcel->getActiveSheet()->setTitle('各种产品出货统计');
    ...
    header('Content-Disposition: attachment;filename="'. $thisyear . '_monthlysummary.xls"');   
