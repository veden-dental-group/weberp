  ...
  if ($_POST["action"] == "save") {
      ...
      foreach ($_POST["oeaarray"] as $oea01){
          if ($_POST["risok" . $oea01] == "Y") {
              ...
              //vd210  訂單
              $soea= "update oea_file set ta_oea005= ta_oea005+$moredays where oea01='$oea01'";
              $erp_sqloea=oci_parse($erp_conn2,$soea);
...
//              }
              ...
              //vd110 工單
              $soea= "update sfb_file set sfb15=sfb15+$moredays where sfb22='$oea01'";
              $erp_sqloea=oci_parse($erp_conn1,$soea);
              $rs=oci_execute($erp_sqloea);
              if ($rs) {
                  $msg.="更新成功!!";  
      ...
      msg($msg);          
      forward("erp_changeduedateonly.php");
  }
...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
  ...
  <?   
      ...
      $soea="select oea01, to_char(ta_oea005,'yyyy-mm-dd') ta_oea005, to_char(oea02,'yyyy-mm-dd') oea02, ta_oea006 from vd210.oea_file " .
            "where 1=1 " . $occfilter . $rxnofilter . $orderdatefilter . " order by ta_oea006 "; 
      $erp_sqloea = oci_parse($erp_conn2,$soea );
      oci_execute($erp_sqloea); 
