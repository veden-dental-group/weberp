<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內每日出貨統計V3";
  include("_data.php");
  include("_erp.php");
  auth("erp_alldaterangecasereportv3_out.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
      ...
      $objPHPExcel = $objReader->load("templates/erp_daterangecasesv3.xls");    
           ...
           from  
             ( select oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select to_char(oga02, 'yyyy-mm-dd') oea02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, oga_file, ima_file
                  where sfb22=oea01 and oea01=oga16 and oga02 between to_date('$bdate','yy/mm/dd') and  to_date('$edate','yy/mm/dd') and sfb05=ima01 )  
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {         
          ...
          #            ->setCellValue('CE'.$y, $v);        
          $objPHPExcel->setActiveSheetIndex(0)        
                              ->setCellValue('AY'.$y, "=V".$y."+AL".$y."+AX".$y);       
     ...
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      $s2="select oea02, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select to_char(oga02,'yyyy-mm-dd') oea02, ta_oea006, ta_oea011, ta_oea004 from oea_file, oga_file where oea01=oga16 and oga02 between to_date('$bdate','yy/mm/dd') 
      ...
      header('Content-Disposition: attachment;filename="DateRangeDeliveryCasesv3.xls"');    
