<?php
  ...
  $key=$bdate.'--'.$edate; 
    ...
    if ($_GET["submit"]=="匯出") {   
        ...
        $q3= "select tdate, mid, mname, pid, pname, qty, unit, price, (qty*price) total from erp_metaltake where tdate='$key' order by mid, pid";
        ...
        while ($row3 = mysql_fetch_array($r3)) {                  
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('F'. $y, $row3["qty"])                        
                        ->setCellValue('G'. $y, $row3["unit"])
                        ->setCellValue('H'. $y, $row3["price"])
                        ->setCellValue('I'. $y, $row3["total"]);  
            if (($y%2)==0){
                $objPHPExcel->getActiveSheet()->getStyle('A'.$y.':H'.$y)->getFill() ->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FFCCFFFF');    
            } 
            $y++;    
        }                       
        $objPHPExcel->getActiveSheet() ->getStyle('A'.($y-1)) ->getBorders() ->getBottom() ->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                        ->setCellValue('B'. $y, '合計')
                        ->setCellValue('I'. $y, '=sum(I' . $oldy . ':I' . ($y-1) . ')'); 
        // Rename sheet
...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
    ...
    <?
      ...
      $r1=mysql_query($q1);
      //
      $s2= "select ina04, gem02, inb04, ima02, inb08, sum(decode(ina00, '1', inb09, '3', (-1)*inb09, 0)) inb09 " .
           ...
           "group by ina04, gem02, inb04, ima02, inb08";
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
          ...
          $query1= "insert into erp_metaltake ( tdate, mid, mname, pid, pname, unit, qty, price ) values ( 
                   '" . $key                . "',  
                   '" . $row2['INA04']      . "', 
                   '" . $row2['GEM02']      . "',  
                   '" . $row2['INB04']      . "',  
      ...
      //
      $q3= "select tdate, mid, mname, pid, pname, unit, qty, price, (qty*price) total from erp_metaltake where tdate='$key' order by mid, pid";
      $r3=mysql_query($q3) or die ('158 erp_metaltake error!!'.mysql_error());    
      $bgkleur = "ffffff";        
      while ($row3 = mysql_fetch_array($r3)) {   
          $i++;
