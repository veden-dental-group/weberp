<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內某客戶每日出貨統計V4";
  include("_data.php");
  include("_erp.php");
  //auth("erp_alldaterangecasereportv4_out.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
      ...
      $objPHPExcel = $objReader->load("templates/erp_daterangecasesv4.xls");    
      $objPHPExcel->setActiveSheetIndex(0)    
                  ->setCellValue('B1', "客戶別每日出貨統計表");
          ...
          (select a.oea02, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select to_char(oga02, 'yyyy-mm-dd') oea02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, oga_file, ima_file
                  where sfb22=oea01 and oea04='$occ01'  and oea01=oga16 and oga02 between to_date('$bdate','yy/mm/dd') and  to_date('$edate','yy/mm/dd') and sfb05=ima01 )  
      ...
      $t=array('    ','8111','8112','811','8121','8122','8123','8124','812','8131','8132','8133','813','8141','8142','8143','814','8151','8152','815','8161','8162','8163','8164','816','81Z1','81Z2','81Z3','81Z4','81Z5','81ZZ','81Z','81', 
               '8211','8212','8213','821','8221','8222','822','8231','8232','8233','823','82Z1','82Z2','82Z3','82Z','82',
               '8311','8312','8313','8314','831','8321','8322','8323','8324','832','83Z1','83');   
               
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {         
          $objPHPExcel->setActiveSheetIndex(0)
                      ->setCellValue('A1', $row2['OEA02']);
      ...
      $objPHPExcel->getActiveSheet()->setTitle('DateRangeCases--by Unit');
      
      //第二個sheet放到貨組數
      $objPHPExcel->setActiveSheetIndex(1)
                  ->setCellValue('A1', "")  
                  ->setCellValue('B1', "每日出貨統計表"); 
                  
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      $s2="select oea02, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           (select oea02,
           ...
           from
            (select to_char(oga02,'yyyy-mm-dd') oea02, ta_oea006, ta_oea011, ta_oea004 from oea_file, oga_file where oea01=oga16 and oea04='$occ01'  and oga02 between to_date('$bdate','yy/mm/dd') 
             and to_date('$edate','yy/mm/dd')  )a ) group by oea02 order by oea02";   
      $erp_sql2 = oci_parse($erp_conn1,$s2 );
      oci_execute($erp_sql2);         
      $y=3;   
      $t=array('    ','A1','A2');
      ...
      header('Content-Disposition: attachment;filename="Dailysum_byCustomer_out.xls"');
