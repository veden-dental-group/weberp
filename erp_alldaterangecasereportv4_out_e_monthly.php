<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內客戶每月到貨合計V4_英文版";
  include("_data.php");
  include("_erp.php");
  //auth("erp_alldaterangecasereportv4_e.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
      ...
      $objPHPExcel = $objReader->load("templates/erp_daterangecasesv4_e.xls"); 
      ...
      $s2="select oea02, 
        ...
        sum(a831) a831, sum(a832) a832, sum(a833) a833  
            ...
            from 
              (select a.oea02, a.ima10, a.ta_oea004,  
               ...
               from  
                 ( select oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                     ( select to_char(oga02, 'yyyy-mm') oea02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, oga_file, ima_file
                      where sfb22=oea01 and oea01=oga16 and oea04='$occ01' and oga02 between to_date('$bdate','yy/mm/dd') and  to_date('$edate','yy/mm/dd') and sfb05=ima01 )  
                   group by oea02, ima01, ima10,ta_oea004) a)
      ...
      $objPHPExcel->setActiveSheetIndex(0)    
                  ->setCellValue('A1', $occ01);               
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {         
          $objPHPExcel->setActiveSheetIndex(0)
                      ->setCellValue('A'.($y), $row2['OEA02'])
                      ->setCellValue('B'.($y), $row2['A8111'])
                      ->setCellValue('C'.($y), $row2['A8121'])
                      ->setCellValue('D'.($y), $row2['A8131'])
                      ->setCellValue('E'.($y), $row2['A8141'])
                      ->setCellValue('F'.($y), $row2['A8151'])
                      ->setCellValue('G'.($y), $row2['A8161'])
                      ->setCellValue('H'.($y), $row2['A81Z1'])
                      ->setCellValue('I'.($y), $row2['A811'])
                      ->setCellValue('J'.($y), $row2['A8211'])
                      ->setCellValue('K'.($y), $row2['A8221'])
                      ->setCellValue('L'.($y), $row2['A8231'])
                      ->setCellValue('M'.($y), $row2['A82Z1'])
                      ->setCellValue('N'.($y), $row2['A821'])
                      ->setCellValue('O'.($y), $row2['A8311'])
                      ->setCellValue('P'.($y), $row2['A8321'])
                      ->setCellValue('Q'.($y), $row2['A83Z1'])
                      ->setCellValue('R'.($y), $row2['A831'])
                      ->setCellValue('S'.($y), "=I".$y."+N".$y."+R".$y);  
          $y++;
      }
      $yy=$y-1;   
      $objPHPExcel->setActiveSheetIndex(0)
                  ->setCellValue('A'.($y), 'TOTAL')
                  ->setCellValue('B'.($y), "=sum(B5:B$yy)")
                  ->setCellValue('C'.($y), "=sum(C5:C$yy)")
                  ->setCellValue('D'.($y), "=sum(D5:D$yy)")
                  ->setCellValue('E'.($y), "=sum(E5:E$yy)")
                  ->setCellValue('F'.($y), "=sum(F5:F$yy)")
                  ->setCellValue('G'.($y), "=sum(G5:G$yy)")
                  ->setCellValue('H'.($y), "=sum(H5:H$yy)")
                  ->setCellValue('I'.($y), "=sum(I5:I$yy)")
                  ->setCellValue('J'.($y), "=sum(J5:J$yy)")
                  ->setCellValue('K'.($y), "=sum(K5:K$yy)")
                  ->setCellValue('L'.($y), "=sum(L5:L$yy)")
                  ->setCellValue('M'.($y), "=sum(M5:M$yy)")
                  ->setCellValue('N'.($y), "=sum(N5:N$yy)")
                  ->setCellValue('O'.($y), "=sum(O5:O$yy)")
                  ->setCellValue('P'.($y), "=sum(P5:P$yy)")
                  ->setCellValue('Q'.($y), "=sum(Q5:Q$yy)")
                  ->setCellValue('R'.($y), "=sum(R5:R$yy)")
                  ->setCellValue('S'.($y), "=sum(S5:S$yy)");

      $objPHPExcel->setActiveSheetIndex(1)    
                    ->setCellValue('A1', $occ01);
                  
      $s2="select oea02, sum(a11) a11, sum(a21) a21 from
            (select oea02, decode(a.ta_oea011,'1',1,'3',1, 0) as a11, decode(a.ta_oea011,'2',1,'3',1, 0) as a21
              from
               (select to_char(oea02,'yyyy-mm-dd') oea02, ta_oea006, ta_oea011 from oea_file where oea04='$occ01' 
                and oea02 between to_date('$bdate','yy/mm/dd') and to_date('$edate','yy/mm/dd')  
               ) a 
            ) group by oea02 order by oea02";  
      $erp_sql2 = oci_parse($erp_conn1,$s2 );
      oci_execute($erp_sql2);         
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {    
          $objPHPExcel->setActiveSheetIndex(1)
                        ->setCellValue('A'.($y), $row2['OEA02'])
                        ->setCellValue('B'.($y), $row2['A11']) 
                        ->setCellValue('C'.($y), $row2['A21']) 
                        ->setCellValue('D'.($y), 0)                                  
...
*/      
      ...
      $filename  = 'email/' . $bdate . '--' .$edate . '_monthlycasessummary_out.xls';
...
<form action="<?=$_SERVER['PHP_SELF'];?>" method="get" name="form1">
  <div align="left">
    <table width="100%"  border="0" cellpadding="4" cellspacing="1" style="border: 1px solid #cccccc">
      <tr><td bgcolor="#eeeeee" width="60">客戶:</td>
          <td>客戶:
