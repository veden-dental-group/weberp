<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內各客戶出貨合計表V3";
  include("_data.php");
  include("_erp.php");
  auth("erp_alldaterangecasereportbycustomerv4_out.php");  
  ...
  if ($_GET["submit"]=="匯出") {  
      ...
      $objPHPExcel = $objReader->load("templates/erp_daterangecasesbycustomerv4.xls");   
      $objPHPExcel->setActiveSheetIndex(0)  
                  ->setCellValue('A1', $bdate.'--'.$edate)  
                  ->setCellValue('B1', "期間內各客戶出貨合计表");  
          ...
          (select a.occ02, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select occ02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select occ02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, oga_file, ima_file, occ_file
                  where sfb22=oea01 and oea01=oga16 and oga02 between to_date('$bdate','yy/mm/dd') and  to_date('$edate','yy/mm/dd') and sfb05=ima01 $clientfilter )  
     ...
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      $s2="select occ02, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select occ02, ta_oea006, ta_oea011, ta_oea004 from oea_file, oga_file, occ_file where oea01=oga16 and  oga02 between to_date('$bdate','yy/mm/dd') 
      ...
      header('Content-Disposition: attachment;filename="DateRangeDeliveryCasesbycustomerv4.xls"');    
