<?php
  session_start();
  $pagetitle = "財務部 &raquo; 每月各區域產品出貨統計表";
  include("_data.php");
  ...
  if ($_GET["submit"]=="匯出") {   
    ...
    $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A1', $thisyear.' 年各区域出貨分析表');       
    ...
    $s2="select gea01, gea02, sum(m1) m1, sum(m2) m2, sum(m3) m3,  sum(m4) m4, sum(m5) m5, sum(m6) m6, sum(m7) m7, sum(m8) m8, sum(m9) m9, sum(m10) m10, sum(m11) m11, sum(m12) m12 from " .
        ...
        "    (select gea01, gea02, to_char(oga02, 'mm') mm, (ogb12 * imaud07) total from ogb_file, oga_file, ima_file, occ_file, gea_file " .
              "where ogb01=oga01 and to_char(oga02,'yyyy')='$thisyear' and ogb04=ima01 and oga04=occ01 and occ20=gea01 )) " .
    ...
    $objPHPExcel->getActiveSheet()->setTitle('每月各區域產品出貨統計表');
