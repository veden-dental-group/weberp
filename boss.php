...
<html>      
    ...
    <body>
      ...
      <?
        ...
        //本月出貨組
        $sout1="select count(*) cases from oga_file, oea_file where to_char(oga02,'yyyy-mm')='$month' and oga16=oea01 $outoccfilter ";           
        $erp_sqlout1 = oci_parse($erp_conn1,$sout1);
        oci_execute($erp_sqlout1);  
        $rowout1 = oci_fetch_array($erp_sqlout1, OCI_ASSOC);
        
        //本月出貨顆
        $sout2="select sum(ogb12*imaud07) units from oga_file, ogb_file, oea_file, ima_file where to_char(oga02,'yyyy-mm')='$month' and oga16=oea01 $outoccfilter and oga01=ogb01 and ogb04=ima01 ";           
        $erp_sqlout2 = oci_parse($erp_conn1,$sout2);
        oci_execute($erp_sqlout2);  
        $rowout2 = oci_fetch_array($erp_sqlout2, OCI_ASSOC);
        
        //本月出貨金額
        $sout3="select sum(tc_ofb14*tc_ofa24) costs from tc_ofb_file, tc_ofa_file where tc_ofb01=tc_ofa01 and to_char(tc_ofa02,'yyyy-mm')='$month' ";
        ...
        //本年出貨組
        $sout4="select count(*) cases from oga_file, oea_file where to_char(oga02,'yyyy')='$year' and oga16=oea01 $outoccfilter ";           
        $erp_sqlout4 = oci_parse($erp_conn1,$sout4);
        oci_execute($erp_sqlout4);  
        $rowout4 = oci_fetch_array($erp_sqlout4, OCI_ASSOC);
        
        //本月出貨顆
        $sout5="select sum(ogb12*imaud07) units from oga_file, ogb_file, oea_file, ima_file where to_char(oga02,'yyyy')='$year' and oga16=oea01 $outoccfilter and oga01=ogb01 and ogb04=ima01 ";           
        $erp_sqlout5 = oci_parse($erp_conn1,$sout5);
        oci_execute($erp_sqlout5);  
        $rowout5 = oci_fetch_array($erp_sqlout5, OCI_ASSOC);
        
        //本月出貨金額
        $sout6="select sum(tc_ofb14*tc_ofa24) costs from tc_ofb_file, tc_ofa_file where tc_ofb01=tc_ofa01 and to_char(tc_ofa02,'yyyy')='$year' ";
          ...
          <table width="100%"  border="0" cellspacing="4" cellpadding="0" STYLE="BORDER: 1px solid #666666">  
            <tr>
              <td width="20%" valign="top">      
                <table width="100%"  border="0" cellspacing="4" cellpadding="0" STYLE="BORDER: 1px solid #666666">  
                  <tr><td><B>月出累計: <B><font color="RED"><?=$rowout1['CASES'];?></font></B> 組</td></tr>                                         
                  <tr><td><B>月出累計: <B><font color="RED"><?=$rowout2['UNITS'];?></font></B> 顆</td></tr>
                  <tr><td><B>月出累計: <B><font color="RED"><?=number_format($rowout3["COSTS"], 0, ".", ",");;?></font></B> 元</td></tr>
                  <tr><td><B>年出累計: <B><font color="RED"><?=$rowout4['CASES'];?></font></B> 組</td></tr>                                         
                  <tr><td><B>年出累計: <B><font color="RED"><?=$rowout5['UNITS'];?></font></B> 顆</td></tr>
                  <tr><td><B>年出累計: <B><font color="RED"><?=number_format($rowout6["COSTS"], 0, ".", ",");;?></font></B> 元</td></tr>
                </table>
              </td>
