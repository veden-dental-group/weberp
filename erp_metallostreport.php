<?php
  ...
  if (is_null($_GET['edate'])) {
    ...
    if ($_GET["submit"]=="匯出") {
        ...
        $s2= "select to_char(sfp02,'mm/dd/yy') sfp02, ima01, ima02, sum(sfpud10) sfpud10, sum(sfpud11) sfpud11, sum(sfpud08) sfpud08, count(*) qty from sfp_file , ( select sfe02, sfe07 from sfe_file group by sfe02, sfe07), ima_file
        ...
        while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('E'. $y, $row2["SFPUD08"])
                        ->setCellValue('F'. $y, $row2["SFPUD10"])
                        ->setCellValue('G'. $y, $row2["SFPUD11"])
                        ->setCellValue('H'. $y, $row2["QTY"]);
            $y++;
        }
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
          ...
          ?>
            <tr bgcolor="#<?=$bgkleur;?>">
                ...
                <td><?=$row2["IMA01"];?></td>
                <td><?=$row2["IMA02"];?></td>
                <td><div align=right><?=number_format($row2["SFPUD08"], 2, ".", ",");?></div></td>
                <td><div align=right><?=number_format($row2["SFPUD10"], 2, ".", ",");?></div></td>
