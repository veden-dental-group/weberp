...
set_time_limit(0);
  ...
  $edate=date('Y-m-d');
      ...
      $s2="select * from (
          ...
          from 
            (select a.gem01, a.gem02, a.oea02, a.ima10, a.ta_oea004,  
             ...
             from  
               ( select gem01, gem02, oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                   ( select gem01, gem02, to_char(oea02, 'yyyy-mm-dd') oea02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, ima_file, gem_file
      ...
      echo "Break 1...\n";                 
      $objPHPExcel = $objReader->load("templates/erp_daterangecasesv4_bymaker.xls");
      echo "Break 2...\n"; 
      ...
      $filename="report/DailyCasesReportv4.xls";
      $objWriter->save($filename);

// modify by mao 2013/08/07
require_once './Classes/phpmailer.php';

$report_name = 'DailyCasesReport';
$mail_reply = 'Dailycasesreport@vedendentalgroup.com';
$mail_to 	= 'Dailycasesreport@vedendentalgroup.com';
...
/*
      ...
      $m->setPriority( 'High' );
      $m->setFrom($mailer_from, "Veden Dental Labs Inc"  );  
      $m->setReplyTo($mailer_replyto, "Veden Dental Labs Inc" );    
      $m->attachFile( $filename, $filename, "application/vnd.ms-excel"); 
      $m->send($mailer_send, $edate . ' Daily Cases Report');
*/
