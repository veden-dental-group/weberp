<?
  ...
  if ($_POST["action"] == "save") {            
      ...
      $occtype=$rowocc['OCCUD10']; //1:PLS 2:CNC 3:UC  NULL:其他客戶
      foreach ($_POST["sfearray"] as $sfe){ 
          ...
          $ssfe2="select sfe01, sfe07, sum(decode(sfe16, null, 0, sfe16) - decode(sfe06, '4', decode(sfe16, null, 0, sfe16), 0 ) - decode(sfe06, '4', decode(ta_sfe003, null, 0, ta_sfe003),0)) sfe16, sum(decode(ta_sfe002, null, 0, ta_sfe002) - decode(sfe06, '4', decode(sfe16, null, 0, sfe16), 0 ) - decode(sfe06, '4', decode(ta_sfe003, null, 0, ta_sfe003),0)) ta_sfe ".
                 "from sfe_file where sfe01='$sfe01' group by sfe01, sfe07";
          ...
          while ($rowsfe2=oci_fetch_array($erp_sqlsfe2, OCI_ASSOC)) {  //同一張工單 可能有兩種金屬以上 
              $sfe16=$rowsfe2['SFE16'];   //鑄造後
              $tasfe=$rowsfe2['TA_SFE'];  //研磨後
              $sfe07=$rowsfe2['SFE07'];   //物料編號
          
              $ssfb="select sfb08, sfb09, sfb22, sfb221 from sfb_file where sfb01='$sfe01'";
              $erp_sqlsfb=oci_parse($erp_conn1,$ssfb);
              $rs3=oci_execute($erp_sqlsfb);
              $rowsfb=oci_fetch_array($erp_sqlsfb, OCI_ASSOC); 
              $sfb08=$rowsfb['SFB08'];     //應生產數量
              $sfb09=$rowsfb['SFB09'];     //入庫數量
              $sfb22=$rowsfb['SFB22'];     //工單號
              ...
              if ($imaud10=='1') {
                  ...
                  //磁牙才有分各種收費方式
                       ...
                       // UC 磁牙加收研磨後10%
                       // PLS NF >= 1*每顆
                  //1:PLS 2:CNC 3:UC  NULL:其他客戶  由於CNC是PLS的其中一些客戶 因為要先判斷CNC   
                  if ($occtype=='3') { //UC 磁牙一律加收研磨後10%   
                      $tasfe = $tasfe * 1.10;   
                  } else if ($occtype=='2') {  //CNC
                      //CNC
                      if        ($imaud10=='3') {          //3:B 要找到顆數 每顆以1.1收                        
                          $tasfe= $sfb08 * 1.1;   
                      } else if ($imaud10=='4') {   // 4:C 要找到顆數 每顆以0.9收     
                          $tasfe= $sfb08 * 0.9;
                      } else if ($imaud10=='5') {     // 5:NF >= 每顆* 1
                          $tasfe=max($sfe16,$sfb08);
                      } else {                  //其餘收鑄造後
                          $tasfe = $sfe16;    
                      }
                   } else if ($occtype=='1') {  //PLS
                      if ($imaud10=='5') {       //5: NF   >= 每顆* 1 
                          $tasfe=max($sfe16,$sfb08);
                      } else {
                          $tasfe = $sfe16 ;
                      }     
...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
  ...
  <?      
      ...
      $ssfe1="select sfbud02, sfe01, sfe02, sfe28, to_char(sfe04,'mm-dd-yyyy') sfe04, sfe07, ima02, imaud10, sfe16 outqty, ta_sfe002, 0 inqty, 0 lostqty, 1 sfetype from sfe_file, sfb_file, ima_file " . 
             ...
             "and sfe01 in ( select tc_ogb002 from tc_ogb_file, tc_oga_file where tc_ogb001=tc_oga001 and tc_oga004='$occ01' and tc_oga002=to_date('$bdate','yy/mm/dd')) "; 
      //退料                                                                                                                    
