<?php
  session_start();
  $pagetitle = "業務部 &raquo; 傳真";
  include("_data.php");
  include("_erp.php");
  auth("erp_clientfax.php");  
  ...
  <div align="left">
    <table width="100%"  border="0" cellpadding="4" cellspacing="1" style="border: 1px solid #cccccc">
      <tr><td bgcolor="#eeeeee">
        ...
        RX #:
        <input name="rxno" type="text" id="rxno" size="20" value="<?=$_GET['rxno'];?>" </input>
        傳真狀態: 
        <select name="isback" id="isback">  
            <option value="">全部</option>
            <option value="N" <? if($_GET['isback']=='N') echo " selected";?> >未回覆</option>
            <option value="Y" <? if($_GET['isback']=='Y') echo " selected";?>>已回覆</option> 
        </select>      
...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
    ...
    <?
      //在VD110 某日以前的訂單 而且有工單的 但沒出貨單的 才算delay
      $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      $edate1=substr($edate,2,2).substr($edate,5,2).substr($edate,8,2);  
      if ($_GET['isback'] =="N") {
          $isback= " and oeaud14 is null"; 
      } else if ($_GET['isback'] =="Y") {  
          $isback= " and oeaud14 is not null";
      } else {
          $isback='';
      }    
      $s1= "select * from " .
           "(select oea01, ta_oea006 rxno, to_char(oea02,'yyyy/mm/dd') indate, to_char(ta_oea005,'yyyy/mm/dd') duedate, oea04, oea01, oea02, oga16, to_char(oeaud13,'yyyy/mm/dd') oeaud13, to_char(oeaud14,'yyyy/mm/dd') oeaud14, oeaud01 from oea_file left join oga_file on oea01=oga16) " . 
           "where ( oeaud13 between to_date('$bdate1','yymmdd') and to_date('$edate1','yymmdd')) $isback and oea04='$occ01' and oga16 is null order by indate,rxno";
      ...
      while ($row1 = oci_fetch_array($erp_sql1, OCI_ASSOC)) {  
          $oea01=$row1['OEA01'];   
          $msg=findcasewithoea01fromtop($oea01, $erp_conn1, $erp_conn2);   
          $s3="select oeb04, oeb06, oeb12, ima1002 from oeb_file, ima_file where oeb01='$oea01' and oeb04=ima01";
          $erp_sql3 = oci_parse($erp_conn,$s3 );
          oci_execute($erp_sql3);
          $i++;   
          while ($row3 = oci_fetch_array($erp_sql3, OCI_ASSOC)) {
              if ($row2['RXNO']==$oldrxno) {
                  $rxno='';
                  $indate='';
                  $duedate='';
                  $ii='';
