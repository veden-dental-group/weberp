<?php
  ...
  if ($_GET["submit"]=="匯出") {   
          ...
          (select a.oea02, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
               ...
               group by oear02, ima01, ima10,ta_oea004) a)
          ...
          //for ($i=1;$i<4;$i++){  //每一個客戶都可能有三筆 new, redo, modify  
              ...
              for ($j=1;$j<=count($t);$j++){
                 ...
                 //             ->setCellValue('A300', $v);  
                  ...
                  //最右邊的合計             
                  $objPHPExcel->setActiveSheetIndex(0)        
                              ->setCellValue('CF'.$yy, "=AV".$yy."+CE".$yy);                
              }     
          //}
          $y++;
      }
      ...
      $s2="select oea02, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select to_char(oea02,'yyyy-mm-dd') oea02, ta_oea006, ta_oea011, ta_oea004 from oea_file, occ_file where oea02 between to_date('$bdate1','yy/mm/dd') and to_date('$edate1','yy/mm/dd') )a ) group by oea02 order by oea02";   
      $erp_sql2 = oci_parse($erp_conn1,$s2 );
      oci_execute($erp_sql2);         
      $y=3;   
      $t=array('    ','A1','A2');
      ...
      header('Content-Disposition: attachment;filename="DailyCases-' . $bdate.'-'.$edate.'.xls"');    
