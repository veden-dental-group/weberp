<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內每日到貨統計V3";
  include("_data.php");
  include("_erp.php");
  #auth("erp_alldaterangecasereportv3.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
           ...
           from  
             ( select oea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select to_char(oea02, 'yyyy-mm-dd') oea02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, ima_file
                  where sfb22=oea01 and oea02 between to_date('$bdate','yy/mm/dd') and  to_date('$edate','yy/mm/dd') and sfb05=ima01 )  
     ...
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      ...
      header('Content-Disposition: attachment;filename="DateRangeCasesv3.xls"');    
