<?php
  session_start();
  $pagetitle = "業務部 &raquo; 出貨統計V4";
  include("_data.php");
  include("_erp.php");
  //auth("erp_clientdailycasereportv4_out.php");  
  ...
  $occ01selected='';   
      ...
      $objPHPExcel = $objReader->load("templates/erp_dailycasesv4.xls");          
      // Add some data      
                      
      $objPHPExcel->setActiveSheetIndex(0)
                  ->setCellValue('A1', "$bdate")                                                                
                  ->setCellValue('C1', "出貨統計表");     
          ...
          (select a.occ07, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select occ07, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select decode(occud03,'Y', occ07, occ01 ) occ07, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, oga_file, ima_file, occ_file 
                  where sfb22=oga16 and oga16=oea01 and oga02=to_date('$bdate1','yy/mm/dd') and sfb05=ima01 and oga04=occ01 $occfilter)  
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
          ...
          for ($i=1;$i<4;$i++){  //每一個客戶都可能有三筆 new, redo, modify  
              for ($j=1;$j<=count($t);$j++){
                  ...
                  //最右邊的合計             
                  $objPHPExcel->setActiveSheetIndex(0)        
                              ->setCellValue('BL'.$yy, "=AH".$yy."+AY".$yy."+BK".$yy);                 
              }     
          }
          $y+=3;
      }
      ...
      $s2="select occ07, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select decode(occud03,'Y', occ07, occ01 ) occ07, ta_oea006, ta_oea011, ta_oea004 from oea_file, oga_file, occ_file 
             where oga02=to_date('$bdate1','yy/mm/dd') and oga04=occ01 and oga16=oea01 $occfilter )a ) group by occ07 order by occ07";   
      $erp_sql2 = oci_parse($erp_conn1,$s2 );
      oci_execute($erp_sql2);         
      $y=3;   
      $t=array('    ','A1','A2');
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {  
          ...
          for ($i=1;$i<4;$i++){  //每一個客戶都可能有三筆 new, redo, modify  橫的要算三行
              for ($j=1;$j<=2;$j++){
                  ...
                  $objPHPExcel->setActiveSheetIndex(1)        
                              ->setCellValue($a.$b.$yy, $v); 
                  $objPHPExcel->setActiveSheetIndex(1)                    //合計
                              ->setCellValue('F'.$yy, '=C'.$yy.'+D'.$yy.'+E'.$yy);               
              }    
          }            
      ...
      $filename='email/DeliveredByClient_' . $bdate . '.xls';
      ...
      $mail->Subject ="=?UTF-8?B?".base64_encode('Veden 每日出貨統計 -- 依客戶別')."?=";//信件标题，解决乱码问题
