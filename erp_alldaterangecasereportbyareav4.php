<?php
  session_start();
  $pagetitle = "業務部 &raquo; 期間內每洲到貨合計表V4";
  include("_data.php");
  include("_erp.php");
  //auth("erp_alldaterangecasereportbyareav3.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
          ...
          (select a.gea02, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select gea02, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
                 ( select gea02, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, ima_file, occ_file, gea_file
                  where sfb22=oea01 and oea02 between to_date('$bdate','yy/mm/dd') and  to_date('$edate','yy/mm/dd') and sfb05=ima01 and oea04=occ01 and occ20=gea01 )  
     ...
     // $bdate1=substr($bdate,2,2).substr($bdate,5,2).substr($bdate,8,2);  
      $s2="select gea02, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select gea02, ta_oea006, ta_oea011, ta_oea004 from oea_file, occ_file, gea_file where oea02 between to_date('$bdate','yy/mm/dd') 
      ...
      header('Content-Disposition: attachment;filename="DateRangeCasesbyareav4.xls"');    
