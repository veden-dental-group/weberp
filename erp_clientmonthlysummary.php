<?php
  session_start();
  $pagetitle = "帳單組 &raquo; Monthly Summary";
  include("_data.php");
  include("_erp.php");
  //auth("erp_clientmonthlysummary.php");  
  ...
  if (($_GET["submit"]=="Excel") or  ($_GET["submit"]=="PDF")) {   
        ...
        $socc= "select occ02,occ18, occ231, occ232, occ261, occ271, occ42 from occ_file where occ01='$occ01'";
        $erp_sqlocc = oci_parse($erp_conn2,$socc );
        oci_execute($erp_sqlocc);  
        $rowocc = oci_fetch_array($erp_sqlocc, OCI_ASSOC);

        $occ02=$rowocc['OCC02'];
        $filename="templates/monthlysummary.xls";
        $currency=$row2['OCC42'];
        ...
        $objPHPExcel ->getProperties()->setCreator( 'Monthly Summary')
                     ->setLastModifiedBy('Monthly Summary')
                     ->setTitle('Monthly Summary')
                     ->setSubject('Monthly Summary')
                     ->setDescription('Monthly Summary')
                     ->setKeywords('Monthly Summary')
                     ->setCategory('Monthly Summary');     
       ...
       // $objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 11); //每頁的1-5row都重複一樣                
        ...
        $bdate1=date('F,d,y', strtotime($bmonth.'-01')) ;
        $edate1=date('F,d,y',mktime(0, 0, 0, $bmonth1 + 1, 1, $byear1) - 1) ;
        $invoicedate=$bdate1. '-' . $edate1;
      
        //$s2="select tc_ofd03, tc_ofdud03, to_char(tc_ofdud13,'Month,dd,yyyy') tc_ofdud13, tc_ofdud04, tc_ofd14, tc_ofc23 " .  
        $s2="select tc_ofc04, occ02, tc_ofc23, sum(tc_ofd14) tc_ofd14 " .  
            "from tc_ofd_file, tc_ofc_file, occ_file where tc_ofc04=occ01 and tc_ofc01=tc_ofd01 and tc_ofc021=$byear1 and tc_ofc022=$bmonth1 and tc_ofc03='$occ01' group by tc_ofc04, occ02, tc_ofc23 order by occ02  ";    
        ...
        while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {  
          $total+=$row2["TC_OFD14"];                            
          $osheet ->setCellValue('A'. $y, $row2["OCC02"])  
                  ->setCellValue('B'. $y, $invoicedate)   
       ...
       // $osheet ->getStyle('C'.$y) ->getNumberFormat() ->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00 ); 
        ...
        $y++;
        $osheet ->mergeCells('A'.$y.':C'.$y);  
        $osheet ->setCellValue('A'.$y, 'k Address : 28/F,Two International Finance Centre,8 Finance Street,Central,HongKong');
        $y++;
        $osheet ->mergeCells('A'.$y.':C'.$y);  
        ...
        $osheet ->setTitle('Monthly Summary');                

        $objPHPExcel->setActiveSheetIndex(0);     
      
 
        header('Content-Type: application/vnd.ms-excel');  
        header('Content-Disposition: attachment;filename="'. 'MonthlySummary_' . $occ01 . '_' . $occ02 . '_' . $bmonth . '.xls"');
