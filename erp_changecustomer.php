<?
  session_start();
  $pagtitle = "IT &raquo; 更改訂單客戶"; 
  include("_data.php");
  include("_erp.php");
  //auth("erp_changecustomer.php");
      ...
      <tr>
        <td bgcolor="#eeeeee"><div align="left">
            ...
            <select name="occ01" id="occ01">  
                <?
                  $socc= "select occ01,occ02 from occ_file order by occ01 ";
                  $erp_sqlocc = oci_parse($erp_conn1,$socc );
                  oci_execute($erp_sqlocc);  
                  while ($rowocc = oci_fetch_array($erp_sqlocc, OCI_ASSOC)) {
                      echo "<option value=" . $rowocc["OCC01"];  
                      if ($_GET["occ01"] == $rowocc["OCC01"]) echo " selected";                  
                      echo ">" . $rowocc['OCC01'] ." -- " .$rowocc["OCC02"] . "</option>"; 
                  }   
                ?>
            </select>   
...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
  ...
  for($i=0; $i<$max; $i++){
      $rxno=$rxarray[$i];
      $soea="select oea01, to_char(oea02,'yyyy/mm/dd') oea02, oea03, oea04, oea17, oea31, oea14, oea99 from oea_file where ta_oea006='$rxno'"; 
      $erp_sqloea = oci_parse($erp_conn2,$soea );
      oci_execute($erp_sqloea);  
