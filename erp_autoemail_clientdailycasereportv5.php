<?php
  ...
  $query="delete from erp_dailycount where iotype='IN' and tdate='$bdate'";
      ...
      (select a.occ07, a.ima10, a.ta_oea004,  
       ...
       from  
         ( select occ07, ima01, ima10, ta_oea004, sum(sfb08*imaud07) sfb08 from 
             ( select decode(occud03,'Y', occ07, occ01 ) occ07, ima01, imaud01 ima10, imaud07, ta_oea004, sfb08 from sfb_file, oea_file, ima_file, occ_file 
              where sfb22=oea01 and oea02=to_date('$bdate','yy/mm/dd') and sfb05=ima01 and oea04=occ01 )  
  ...
  while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
      ...
      $query .= " ) values ( '$uuid', '$bdate', 'U', 'IN', '$occ01', '$occ02' "; 
      for ($j=1;$j< $c; $j++){ 
          for ($i=1;$i<4;$i++){      
              $value=$row2['A'.$t[$j].$i];      
              $query .= ",$value";   
  ...
  $s2="select occ07, sum(a11) a811, sum(a12) a812, sum(a13) a813, sum(a21) a821, sum(a22) a822, sum(a23) a823, sum(a31) a831, sum(a32) a832, sum(a33) a833 from
       ...
       from
        (select decode(occud03,'Y', occ07, occ01 ) occ07, ta_oea006, ta_oea011, ta_oea004 from oea_file, occ_file where oea02=to_date('$bdate','yy/mm/dd') and oea04=occ01 )a ) group by occ07 order by occ07";   
  $erp_sql2 = oci_parse($erp_conn1,$s2 );
  oci_execute($erp_sql2);         
  $y=3;   
  $t=array('    ','81','82','83');
  ...
  while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {  
      ...
      $query .= " ) values ( '$uuid', '$bdate', 'C', 'IN', '$occ01', '$occ02' "; 
      for ($j=1;$j< $c; $j++){ 
          for ($i=1;$i<4;$i++){      
              $value=$row2['A'.$t[$j].$i];      
              $query .= ",$value";   
  ...
  $objPHPExcel->setActiveSheetIndex(0)
              ...
              ->setCellValue('B1', "到貨統計表")
              ->setCellValue('A2', "單位：組數") ;  
  $objPHPExcel->setActiveSheetIndex(0)->setTitle('每日到貨組數'); 
  
  $query="select occ02, a811, a821, a831 from erp_dailycount where qtype='C' and iotype='IN' and tdate='$bdate' order by occ02 ";
  ...
  $objPHPExcel->setActiveSheetIndex(1)          
              ->setCellValue('A1', "單位：組數") ;  
  $objPHPExcel->setActiveSheetIndex(1)->setTitle('30日內到貨組數'); 
  ...
  $query .= " from (select occ02, tdate, (a811+a821+a831) a8 from erp_dailycount where qtype='C' and iotype='IN' and tdate <= '$tdate01' and tdate >= '$tdate30') a ) b group by occ02 order by occ02 ";
...
$filename='email/' . $bdate . '_VD_DailyCasesReceived.xls.xls';    
$objWriter->save($filename);   


require_once './Classes/phpmailer.php';

$report_name = 'Daily Cases Report By Client';   
$mail_reply   = 'frank@vedendentalgroup.com';  
$mail1_to     = 'casesreceived@vedendentalgroup.com';
$mail2_to     = 'wenhsien.cheng@gmail.com';    

//$mail1_to ='frank@vedendentalgroup.com';
//$mail2_to ='frank@vedendentalgroup.com';   


$mail->Subject ="=?UTF-8?B?".base64_encode('Veden 每日到貨統計 -- 依客戶別')."?=";//信件标题，解决乱码问题
//$mail->Subject = "Veden 每日到貨統計 -- 依客戶";
$mail->Body = " ";
$mail->AddReplyTo($mail_reply, 'Veden Dental Group');
$mail->AddAddress($mail1_to, 'Veden Dental Group');
$mail->AddAddress($mail2_to, 'Veden Dental Group');     
//$mail->AddAddress($mail3_to, 'Veden Dental Group');
