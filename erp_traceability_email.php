...
if (is_null($_GET['email'])) {
  $email = 'report@vedendentalgroup.com';
} else {
  $email=$_GET['email'];
}
...
if ($_GET["submit"]=="E-Mail") { 
    ...
    $template_file=  "templates/traceability_" . $occ01 . ".xls";
    if (!(file_exists($template_file))) {
    	$template_file=  "templates/traceability_VEDEN.xls";
    }
	...
	*/
    ...
    while ($row   = mysql_fetch_array($result) ){   
        ...
        $objPHPExcel = $objReader->load($template_file);  
            // Set properties
        $objPHPExcel ->getProperties()->setCreator("ERPAP")
                     ->setLastModifiedBy("ERPAP")
        ...
        //第二份 modified by Mao 20131001
        $objPHPExcel->setActiveSheetIndex(0)     
                    ->setCellValue('E19', $row['rxno'])
                    ->setCellValue('E20', $row['dentist'])   
                    ->setCellValue('E21', $row['patient'])   
                    ->setCellValue('C22', $row['pename']) 
                    ->setCellValue('C23', $row['tooth'])   
                    ->setCellValue('C24', $row['shade']);               
		/*
        //第二份            
        $objPHPExcel->setActiveSheetIndex(0)     
	...
	$mail->Subject = "Veden Dental Group Daily Report -- $report_name ";
	$mail->Body = "Veden Dental Group Daily Report -- $report_name ";
	$mail->AddReplyTo($mail_reply, 'Veden Dental Group');
	$mail->AddAddress($mail_to, 'Veden Dental Group');
	// $mail->AddAddress('tower@vedendentalgroup.com', 'Veden Dental Group');
	// $mail->AddAddress('amanism@gmail.com', 'Veden Dental Group');

    // $totalpage=floor($j/100)+1;
    // $page=1;
    for ($x=1; $x<=$j; $x++) {          
		$mail->AddAttachment($filename[$x]);
		/*
    ...
    $m->setMessage($regards);
    $m->setPriority( 'High' );
    $m->setFrom( "report@vedendentalgroup.com", "Veden Dental Labs Inc"  );
    $m->setReplyTo( "report@vedendentalgroup.com", "Veden Dental Labs Inc" );  
...
if ($_GET["submit"]=="計算") {   //重算期間內 某客戶的全部資料 
	...
	// modified by mao 09/05/2013 : add franch name with imc_file 
	// 09/13 change oga02 to oea02 for different date range filter.
	// 10/11 change main table from oga to oea for new filter.
	// new sql
	$s1 = "
	select oea01 ogb31, oeb03 ogb32, ta_oea006 ogaud02, to_char(oea02, 'yyyy-mm-dd') oga02, 
	...
	( select distinct tc_fro002 from tc_fro_file where tc_fro001='$occ01' or tc_fro001='MISC' ), 
	...
	and oeb03=sfb221 
	and oeb04=ima01 
	order by oea02, ta_oea006
	";

/*
    echo $s1="select ogb31, ogb32, ogaud02, to_char(oea02, 'yyyy-mm-dd') oga02, occ01, occ02, sfb01, sfb82, ta_oea002, ta_oea003, ta_oea046, ta_oea047, ta_oea048, ima01, ima02, ima1002, imc04, oebud03 ".
        "from oga_file, occ_file, ogb_file, ( select distinct tc_fro002 from tc_fro_file where tc_fro001='$occ01' or tc_fro001='MISC' ), sfb_file, oea_file, oeb_file, " . //取出本客戶或MICS客戶有設定的產品清單
		"(select  ima01, ima02, ima1002, imc04 from ima_file left join imc_file on ima01=imc01 and imc02='FR' and imc03='1' )  " .
        "where oea02 between to_date('$bdate','yy/mm/dd') and to_date('$edate','yy/mm/dd') and oga04='$occ01' and oga04=occ01 " .
...
*/
    ...
    while ($row1 = oci_fetch_array($erp_sql1, OCI_ASSOC)) {          //取出單頭所有資料
		...
		// modified by mao 09/05/2013
		if($occ01=='E172001' || $occ01=='E143001')
        	$pename       =$row1['IMC04'];
		else
        	$pename       =$row1['IMA1002'];
		...
		/*
          echo $s4 = "select tc_frl004, ima02, ima1002, imc04 ".
				"from tc_frl_file, ".
				"(select  ima01, ima02, ima1002, imc04 from ima_file left join imc_file on ima01=imc01 and imc02='FR' and imc03='1' )  ".
				"where tc_frl001='$pcode' and tc_frl010='$tc_frl010' and tc_frl002='$tc_frl002' and tc_frl003='$tc_frl003' and tc_frl005='$tc_frl005' and tc_frl004=ima01 " ;  
