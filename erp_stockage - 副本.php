<?php
  ...
  if ($code=='') {
    ...
    $rvvfilter =" and rvv31='$code' ";
    $sfefilter =" and sfe07='$code' ";
    $inbfilter =" and inb04='$code' ";   
    $impfilter =" and imp03='$code' "; 
    $imqfilter =" and imq05='$code' ";   
    $ogbfilter =" and ogb04='$code' "; 
  }
  ...
  if (($_GET["submit"]=="Excel") or  ($_GET["submit"]=="PDF")) {   
        ...
        $i=0;
        $y=5;
        $query7="select * from stock where countdate='$edate' $codefilter and ( qty0!=0 or qty1>0 or qty2>0 or qty3>0 or qty4>0 or qty5>0 or qty6>0) order by code " ;  
  ...
  <div align="left">
    <table width="100%"  border="0" cellpadding="4" cellspacing="1" style="border: 1px solid #cccccc">
      <tr><td bgcolor="#eeeeee">
        ...
        物料編號:
        <input name="code" type="text" id="code" size="12" value="">&nbsp;&nbsp; 
        重新計算:
        <input type="checkbox" id="recount" name="recount" value="Y" <? if($_GET['recount']=='Y') echo " checked";?>>     
        &nbsp;&nbsp;  
...
<table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
    <tr>
        ...
        <th>品代</th>         
        <th>品名</th>  
        <th>1天~180天 (X <= <?=$edate1;?>)</th>  
        <th>181天~一年 (<?=$edate1;?> < X <= <?=$edate2;?> )</th> 
        <th>一年~二年 (<?=$edate2;?> < X <= <?=$edate3;?> )</th>  
        <th>二年~三年 (<?=$edate3;?> < X <= <?=$edate4;?> )</th>  
        <th>三年~四年 (<?=$edate4;?> < X <= <?=$edate5;?> )</th>  
        <th>四年以上  ( X < <?=$edate6;?> )</th>  
        <th>庫存不足</th>   
        <th>單位</th>          
    </tr>
    <?  
     if ($_GET['recount']=='Y') {
          ...
          //先刪除同一個統計日
          $query2= "delete from stockin where countdate='$edate' $codefilter ";
          ...
          $s1="select to_char(rvu03,'yyyy-mm-dd') rvu03, rvv31, ima02, rvvud02, sum(rvvud07) rvvud07 from rvu_file, rvv_file, ima_file " . 
              "where rvuconf='Y' and rvu00='1' and rvu03<=to_date('$edate','yy/mm/dd') and rvu01=rvv01 and rvv31=ima01 and substr(ima06,1,1)!='9' $rvvfilter group by rvu03,rvv31,ima02,rvvud02 ";
          $erp_sql1 = oci_parse($erp_conn1,$s1 );
          oci_execute($erp_sql1);     
          ...
          $s2="select to_char(sfe04,'yyyy-mm-dd') sfe04, sfe07, ima02, sfe17, sum(sfe16) sfe16 from sfe_file, ima_file " .
              "where sfe06='4' and sfe04<=to_date('$edate','yy/mm/dd') and sfe07=ima01 and substr(ima06,1,1)!='9' $sfefilter group by sfe04,sfe07,ima02,sfe17 ";
          ...
          //雜收 3
          $s2="select to_char(ina02,'yyyy-mm-dd') ina02, inb04, ima02, inb08, sum(inb09) inb09 from ina_file, inb_file, ima_file " .
              "where inaconf='Y' and ina00='3' and ina02<=to_date('$edate','yy/mm/dd') and ina01=inb01 and inb04=ima01 and substr(ima06,1,1)!='9' $inbfilter group by ina02,inb04,ima02,inb08 ";
          $erp_sql2 = oci_parse($erp_conn1,$s2 );
          oci_execute($erp_sql2);     
          while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
              $ina02=$row2['INA02'];   //日期
          ...
          $s2="select to_char(imo02,'yyyy-mm-dd') imo02, imp03, ima02, imp05, sum(imp04) imp04 from imo_file, imp_file, ima_file " .
              "where imoconf='Y' and imo02<=to_date('$edate','yy/mm/dd') and imo01=imp01 and imp03=ima01 and substr(ima06,1,1)!='9' $impfilter group by imo02,imp03,ima02,imp05 ";
          $erp_sql2 = oci_parse($erp_conn1,$s2 );
          oci_execute($erp_sql2);     
          while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
              $imo02=$row2['IMO02'];   //日期
          ...
          //發料 2
          $s2="select to_char(sfe04,'yyyy-mm-dd') rvu03, sfe07 rvv31, sum(sfe16) rvv17 from sfe_file where sfe06='1' and sfe04<=to_date('$edate','yy/mm/dd') $sfefilter group by sfe04, sfe07";
          $erp_sql2 = oci_parse($erp_conn1,$s2 );
          oci_execute($erp_sql2);     
          while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
              $rvu03=$row2['RVU03'];   //日期
          ...
          //雜發 3
          $s2="select to_char(ina02,'yyyy-mm-dd') rvv03, inb04 rvv31, sum(inb09) rvv17 from ina_file, inb_file where inaconf='Y' and ina00='1' and ina02<=to_date('$edate','yy/mm/dd') and ina01=inb01 $inbfilter group by ina02,inb04 ";
          $erp_sql2 = oci_parse($erp_conn1,$s2 );
          oci_execute($erp_sql2);     
          while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
              $rvu03=$row2['RVV03'];   //日期
          ...
          //還料 4
          $s2="select to_char(imr02,'yyyy-mm-dd') rvv03, imq05 rvv31, sum(imq07) rvv17 from imr_file, imq_file where imrconf='Y' and imr02<=to_date('$edate','yy/mm/dd') and imr01=imq01 $imqfilter group by imr02, imq05 ";
          $erp_sql2 = oci_parse($erp_conn1,$s2 );
          oci_execute($erp_sql2);     
          while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
              $rvu03=$row2['RVV03'];   //日期
          ...
          $s2="select to_char(oga02,'yyyy-mm-dd') rvv03, ogb04 rvv31, sum(ogb12) rvv17 from oga_file, ogb_file " .
              "where oga905 is null and ogaconf='Y' and oga02<=to_date('$edate','yy/mm/dd') and oga01=ogb01 $ogbfilter group by oga02, ogb04 ";
          $erp_sql2 = oci_parse($erp_conn1,$s2 );
          oci_execute($erp_sql2);     
          while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) { 
              $rvu03=$row2['RVV03'];   //日期
          ...
          //先刪除stockin 同一個統計日
          $query2= "delete from stock where countdate='$edate' $codefilter ";
          $result2= mysql_query($query2) or die ('1180 Stockout Deleted error. ' .mysql_error());   
          
          
          $query1="select code, name, unit, indate, sum(qty) qty from stockin where countdate='$edate' $codefitler group by code, name, unit, indate order by code,indate ";
          ...
          while ($row1= mysql_fetch_array($result1)) {
              ...
              $unit=$row1['unit'];
              //先檢查本料號有記錄否
              $query2="select * from stock where countdate='$edate' $codefilter ";
              ...
              $result3= mysql_query($query3) or die ('1304 Stock Updated error. ' .mysql_error()); 
              commit;   
          }  
          
          //自stockout中取出物料的合計出庫數
          $query4="select code, sum(qty) qty from stockout where countdate='$edate' $codefilter group by code order by code ";
