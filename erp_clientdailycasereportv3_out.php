<?php
  session_start();
  $pagetitle = "業務部 &raquo; 出貨統計V3";
  include("_data.php");
  include("_erp.php");
  auth("erp_clientdailycasereportv3_out.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
      ...
      $objPHPExcel = $objReader->load("templates/erp_dailycasesv3.xls");          
      // Add some data      
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
          ...
          for ($i=1;$i<4;$i++){  //每一個客戶都可能有三筆 new, redo, modify  
              for ($j=1;$j<=count($t);$j++){
                  ...
                  //最右邊的合計             
                  $objPHPExcel->setActiveSheetIndex(0)        
                              ->setCellValue('BA'.$yy, "=W".$yy."+AN".$yy."+AZ".$yy);                 
              }     
          }
          $y+=3;
      }
      ...
      header('Content-Disposition: attachment;filename="DailyDeliveryCasesv3-' . $bdate.'.xls"');    
