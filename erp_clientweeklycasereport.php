<?php
  ...
  include("_data.php");
  include("_erp.php");
  //auth("erp_clientweeklycasereport.php");  
  ...
  if ($_GET["submit"]=="匯出") {   
      ...
      $s2="select occ07, 
        ...
        sum(a931b1) a931b1, sum(a931b2) a931b2, sum(a931b3) a931b3,  
        ...
        from 
          (select a.occ07, a.ima10, a.ta_oea004,  
           ...
           from  
             ( select occ07, ima01, ima10, ta_oea004, sum(sfb08) sfb08 from 
                 ( select decode(occud03,'Y', occ07, occ01 ) occ07, ima01, ima10, ta_oea004, sfb08 from sfb_file, oea_file, ima_file, occ_file 
                  where sfb22=oea01 and oea02 between to_date('$bdate','yy/mm/dd') and to_date('$edate','yy/mm/dd') and sfb05=ima01 and oea04=occ01 $occfilter)  
      ...
      $t=array('    ','9311','931B','9312','9313','9314','9315','9316','9317','9318','9319','931A','931','9321','9322','932',
      ...
      while ($row2 = oci_fetch_array($erp_sql2, OCI_ASSOC)) {
          ...
          for ($i=1;$i<4;$i++){  //每一個客戶都可能有三筆 new, redo, modify  
              for ($j=1;$j<=count($t);$j++){
                  ...
                  //最右邊的合計             
                  $objPHPExcel->setActiveSheetIndex(0)        
                              ->setCellValue('CD'.$yy, "=AT".$yy."+CC".$yy);             
              }     
          }
          $y+=3;
      }
      ...
      $objPHPExcel->getActiveSheet()->setTitle('WeeklyCases--by Unit');
      ...
      $s2="select occ07, sum(a11) a11, sum(a12) a12, sum(a13) a13, sum(a21) a21, sum(a22) a22, sum(a23) a23 from
           ...
           from
            (select decode(occud03,'Y', occ07, occ01 ) occ07, ta_oea006, ta_oea011, ta_oea004 from oea_file, occ_file where oea02 between to_date('$bdate','yy/mm/dd') 
             and to_date('$edate','yy/mm/dd') and oea04=occ01 $occfilter )a ) group by occ07 order by occ07";   
      $erp_sql2 = oci_parse($erp_conn1,$s2 );
      oci_execute($erp_sql2);         
      $y=3;   
      $t=array('    ','A1','A2');
      ...
      header('Content-Disposition: attachment;filename="WeeklyCases.xls"');    
