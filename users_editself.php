<?
  ...
  if ($_POST["action"] == "save") {    
      ...
      if ($_POST["password1"] != "") {
        $queryul = "update users set password='". md5(safetext($_POST["password1"])) . "' where guid   = '" . safetext($_POST["guid"])         . "' limit 1";
  ...
  $queryu = "select * from users where account='" . ($_SESSION["account"]) . "' limit 1";
  ...
  <table width="100%"  border="0" cellpadding="2" cellspacing="1" class="table4">
    ...
    <tr>
      ...
      <td><input type="hidden" name="action" value="save">
          <input type="hidden" name="account" value="<?=$rowu["account"];?>"> 
          <input type="hidden" name="guid" value="<?=safetext($rowu["guid"]);?>">
